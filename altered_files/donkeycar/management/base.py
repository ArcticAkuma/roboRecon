...
class CreateCar(BaseCommand):
    ...
    def create_car(self, path, template='complete', overwrite=False):
        ...
        if os.path.exists(car_app_path) and not overwrite:
            ...
            os.chmod(os.path.join(path, 'record.bash'), stat.S_IEXEC)
            shutil.copyfile(os.path.join(TEMPLATES_PATH, 'replay.bash'), os.path.join(path, 'replay.bash'))
            os.chmod(os.path.join(path, 'replay.bash'), stat.S_IEXEC)
